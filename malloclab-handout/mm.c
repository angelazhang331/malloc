...
static block_t *head;	/* added head pointer for the start of the free list */
...
void insert_efl(block_t *block);
void remove_efl(block_t *block);

/*
 * mm_init - Initialize the memory manager
 * 
 * the heap is still initialized in the same way as the implicit list
 * new pointers for the efl
 * now utilized next and prev pointers in the blocks
 *
 */
/* $begin mminit */
...
int mm_init(void) {
    ...
    /* initialized first free block pointers to start efl */
    init_block->body.next = NULL;
    init_block->body.prev = NULL;

    /* set head pointer to point to the first free block */
    head = init_block;
...
void *mm_malloc(size_t size) {
	...
	//printf("malloc heap extended\n");
        place(block, asize);
        return block->body.payload;
    }

    //printf("malloc heap cannot be extended\n");
...
void mm_free(void *payload) {
    //printf("free called\n");
    /* get a pointer to the start of the info of the block */
    block_t *block = payload - sizeof(header_t);

    /* set the block to be freed */
     block->allocated = FREE;

    /* acces the footer of the block being freed */
    ...
    /* attempt to coalesce the newly freed block with adjacent free blocks */
    coalesce(block);
}
 ...
 */
/* do last,, needs list to work first */
...
static void place(block_t *block, size_t asize) {
	...
	/* add new free block to efl */
	insert_efl(new_block);
	
	/* remove alloc block from ef1 */
	remove_efl(block);
...
static block_t *find_fit(size_t asize) {
    ...
    /* iterate through the efl until a free block of the right size is found */
...
static block_t *coalesce(block_t *block) {
    ...
    /* if both are allocated, then there is nothing to coalesce so the original block is returned */
    ...
    if (prev_alloc && next_alloc) { /* Case 1 */
        ...
        /* update the efl */
        remove_efl((block_t*)next_header);
    }
    ...
    /* if both previous and next are free */
    /* resulting block has a size of all three ,, ignoring header and footer of current block */
...
/* manages the pointers for inserting a block into the efl */
/* after heap has been extended or block already freed */

void insert_efl(block_t *block) {
    //printf("inserting empty block...\n");
    ...
    /* if the list was not empty, add to the front */
    else {
...
/* manages the pointers after removing a block from the efl */
/* used when a free block becomes allocated */

void remove_efl(block_t *block) {
    //printf("removing block...\n");
    block_t *next = block->body.next;
    block_t *prev = block->body.prev;    

    /* if the block being removed is the only block in the list */
    if (prev == NULL && next == NULL) {
        head = NULL;
        //printf("efl is empty )):");
    }

    /* if the block being removed is the last block in the list */
    else if(next == NULL) {
        prev->body.next = NULL;
	//printf("last block removed\n");
    }

    /* if the block being removed is the first block in the list */
    else if(prev == NULL) {
        head = next;
        next->body.prev = NULL;
        //printf("first block removed\n");
    }

    /* if the block is in the center of the efl */
    else {
        next->body.prev = prev;
        prev->body.next = next;
	//printf("some block removed\n");
    }
}
